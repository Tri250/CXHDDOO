...
namespace Client.MirScenes
{
    public class SelectScene : MirScene
    {
        ...
        {
            CMain.SaveError(DXManager.PrintParentMethod() + $"SelectScene开始初始化");
            Client.Utils.ResolutionTrace.LogClientState("SelectScene.ctor", $"Characters={characters?.Count ?? 0}");
            ...
            Size backgroundDesignSize = Background.TrueSize;
            int designWidth = backgroundDesignSize.Width > 0 ? backgroundDesignSize.Width : 800;
            int designHeight = backgroundDesignSize.Height > 0 ? backgroundDesignSize.Height : 600;
            int ScaleX(int x) => (int)Math.Round(x * (double)Settings.ScreenWidth / designWidth);
            int ScaleY(int y) => (int)Math.Round(y * (double)Settings.ScreenHeight / designHeight);
            ...
            Title = new MirImageControl
            {
                ...
                Parent = this,
                Location = new Point(ScaleX(358), ScaleY(15))
				//Location = new Point(468, 20)
            };

            ServerLabel = new MirLabel
            {
                Location = new Point(ScaleX(322), ScaleY(44)),
            ...
            {
                Location = new Point(ScaleX(447), ScaleY(122)),
                Parent = Background,
            ...
            {
                Location = new Point(ScaleX(447), ScaleY(226)),
                Parent = Background,
                Sound = SoundList.ButtonA,
            };
            ...
            {
                Location = new Point(ScaleX(447), ScaleY(330)),
                Parent = Background,
                Sound = SoundList.ButtonA,
            };
            ...
            {
                Location = new Point(ScaleX(447), ScaleY(434)),
                Parent = Background,
                Sound = SoundList.ButtonA,
            };
            ...
            {
                Location = new Point(ScaleX(140), ScaleY(509)),
                Parent = Background,
            ...
            {
                Location = new Point(ScaleX(-80), ScaleY(-1)),
        ...
        public void StartGame(S.StartGame p)
        {
            ...
            switch (p.Result)
            {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    var targetResolution = Settings.Resolution;
                    if (p.Resolution > 0 && (targetResolution == 0 || p.Resolution < targetResolution))
                        targetResolution = p.Resolution;

                    Client.Utils.ResolutionTrace.LogClientState("SelectScene.StartGame", $"ServerResolution={p.Resolution}, LocalResolution={Settings.Resolution}, TargetResolution={targetResolution}");

                    Settings.ApplyResolutionSetting(targetResolution);
                    var resolutionSize = Settings.GetResolutionSize();
                    CMain.SetResolution(resolutionSize.Width, resolutionSize.Height);
                    Client.Utils.ResolutionTrace.LogClientState("SelectScene.StartGame", $"After SetResolution size={resolutionSize.Width}x{resolutionSize.Height}");

                    ActiveScene = new GameScene();
                    Client.Utils.ResolutionTrace.LogClientState("SelectScene.StartGame", "After ActiveScene = new GameScene");
                    Dispose();
                    break;
            }
        }
