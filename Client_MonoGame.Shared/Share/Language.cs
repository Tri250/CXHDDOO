...
public class GameLanguage
{
    ...
    public static string Welcome = "欢迎来到 {0} 服务器",
                         ...
                         AllowingGroupRequests = "组队请求被认可",
                         BlockingGroupRequests = "组队请求被拒绝";
    ...
    public static string LoadClientMagic(string Name)
    {
        string languageIniPath = EnsureClientLanguageFile(Settings.LanguageFilePath);
        InIReader reader = new InIReader(languageIniPath);
        return reader.ReadString("Language", Name, "", false);
    }


    public static void LoadClientLanguage(string languageIniPath)
    {
        languageIniPath = EnsureClientLanguageFile(languageIniPath);
    ...
    private static string EnsureClientLanguageFile(string languageIniPath)
    {
        string resolvedPath = ClientResourceLayout.ResolvePath(languageIniPath);
        if (File.Exists(resolvedPath))
            return resolvedPath;

        if (ClientResourceLayout.TryHydrateFileFromPackage(resolvedPath, out string availablePath) && File.Exists(availablePath))
            return availablePath;

        ClientResourceLayout.ReportMissingPackageRequest(resolvedPath);
        SaveClientLanguage(resolvedPath);
        return resolvedPath;
    }
    ...
    public static void LoadServerLanguage(string languageIniPath)
    {
        ...
        GameLanguage.AllowingGroupRequests = reader.ReadString("Language", "AllowingGroupRequests", GameLanguage.AllowingGroupRequests);
        GameLanguage.BlockingGroupRequests = reader.ReadString("Language", "BlockingGroupRequests", GameLanguage.BlockingGroupRequests);
    }
    ...
    public static void SaveServerLanguage(string languageIniPath)
    {
        ...
        reader.Write("Language", "AllowingGroupRequests", GameLanguage.AllowingGroupRequests);
        reader.Write("Language", "BlockingGroupRequests", GameLanguage.BlockingGroupRequests);
    }
}
