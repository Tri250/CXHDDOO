...
class SceneClassifier(context: Context) {
    ...
    private val computeExecutor = Executors.newFixedThreadPool(2) { r ->
        Thread(r, "SceneClassifier-worker").apply { isDaemon = true }
    }
    ...
    fun classify(
        ...
        runCatching {
            labeler.process(image)
                .addOnSuccessListener(computeExecutor) { labels ->
                    runCatching {
                ...
                .addOnFailureListener(computeExecutor) {
                    runCatching {
                        val pixelVotes = computePixelHeuristicVotes(bitmap)
                        val colorHistVotes = computeColorHistogramVotes(bitmap)
    ...
    private fun computePixelHeuristicVotes(bitmap: Bitmap): FloatArray {
        ...
        return try {
            ...
            val colorHistogram = IntArray(64)
