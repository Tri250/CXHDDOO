...
{$IFEND}
  ...
  // === 新增 BUFF/DEBUFF 状态常量 ===
  STATUS_BUFF_START = 14;        // BUFF起始索引
  STATUS_BUFF_ATKUP = 14;        // 攻击力提升
  STATUS_BUFF_DEFUP = 15;        // 防御力提升
  STATUS_BUFF_SPEEDUP = 16;      // 速度提升
  STATUS_BUFF_CRITUP = 17;       // 暴击率提升
  STATUS_BUFF_VAMPIRE = 18;      // 吸血效果
  STATUS_BUFF_SHIELD = 19;       // 护盾效果
  STATUS_BUFF_IMMUNE = 20;       // 免疫控制
  STATUS_BUFF_REFLECT = 21;      // 伤害反弹
  STATUS_BUFF_CONTINUOUSHEAL = 22; // 持续回血
  STATUS_BUFF_THORNS = 23;       // 荆棘光环
  STATUS_DEBUFF_SLOW = 24;       // 减速
  STATUS_DEBUFF_SILENCE = 25;    // 沉默
  STATUS_DEBUFF_WEAK = 26;       // 虚弱
  STATUS_DEBUFF_BLEED = 27;      // 流血
  STATUS_DEBUFF_BURN = 28;       // 灼烧
  STATUS_DEBUFF_FREEZE = 29;     // 冰冻
...
//  MAXMAKEMAGICCOUNT = 10;
  ...
  // === 新增系统消息常量 ===
  SM_BUFFUPDATE = 5366;          // BUFF状态更新
  SM_BUFFADDEFFECT = 5367;       // BUFF特效添加
  SM_BUFFDELEFFECT = 5368;       // BUFF特效移除
  SM_DUNGEONENTER = 5369;        // 进入副本
  SM_DUNGEONEXIT = 5370;         // 退出副本
  SM_DUNGEONINFO = 5371;         // 副本信息
  SM_DUNGEONCOUNTDOWN = 5372;    // 副本倒计时
  SM_ACTIVITYNOTICE = 5373;      // 活动公告
  SM_ACTIVITYSTART = 5374;       // 活动开始
  SM_ACTIVITYEND = 5375;         // 活动结束
  SM_ACTIVITYRANK = 5376;        // 活动排名
  SM_MASTERINFO = 5377;          // 师徒信息
  SM_MASTERAPPRENTICE = 5378;    // 师徒关系
  SM_MASTERQUEST = 5379;         // 师徒任务
  SM_AFFIXINFO = 5380;           // 词缀信息
  SM_AFFIXREFRESH = 5381;        // 词缀刷新
  SM_GUILDTECHINFO = 5382;       // 行会科技信息
  SM_GUILDTECHUPGRADE = 5383;    // 行会科技升级
  SM_GUILDSTORAGE = 5384;        // 行会仓库
  SM_AUTOFIGHTSTATE = 5385;      // 自动战斗状态
  SM_SEASONINFO = 5386;          // 赛季信息
  SM_SEASONRANK = 5387;          // 赛季排名
  SM_SEASONREWARD = 5388;        // 赛季奖励
...
//    boEvent: Boolean;
  ...
  TBuffInfo = packed record
    nBuffID: Word;              // BUFF唯一ID
    nBuffType: Word;            // BUFF类型编号
    sBuffName: string[20];      // BUFF名称
    BuffType: TBuffType;        // BUFF或Debuff
    nDuration: LongWord;        // 剩余持续时间(ms)
    nMaxDuration: LongWord;     // 最大持续时间
    nInterval: LongWord;        // tick间隔(ms)
    dwLastTick: LongWord;       // 上次tick时间
    nValue: Integer;            // 数值参数
    nValue2: Integer;           // 数值参数2
    nSourceObj: Integer;        // 来源对象ID
    nEffectID: Word;            // 特效ID
    nOverlay: Byte;             // 叠加层数
    nMaxOverlay: Byte;          // 最大叠加层数
    boPermanent: Boolean;       // 是否永久BUFF
  ...
  TGCDInfo = packed record
    nSkillID: Word;             // 技能ID
    dwLastCastTime: LongWord;   // 上次施放时间
    nGCDTime: LongWord;         // GCD时间(ms)
    nGlobalGCDTime: LongWord;   // 全局GCD时间(ms)
  end;
  ...
  TAffixEntry = packed record
    nAffixID: Word;             // 词缀ID
    sAffixName: string[20];     // 词缀名称
    nQuality: Byte;             // 品质等级 0-3
    nAttrType: Byte;            // 属性类型 0=DC,1=MC,2=SC,3=AC,4=MAC,5=HP,6=MP,7=HitSpeed,8=Deadliness,9=Vampire
    nMinValue: Integer;         // 最小属性值
    nMaxValue: Integer;         // 最大属性值
    nWeight: Integer;           // 出现权重
    nEquipType: Byte;           // 装备类型限制 0=全部,1=武器,2=衣服,3=首饰,4=头盔
    nLevelMin: Integer;         // 装备等级下限
    nLevelMax: Integer;         // 装备等级上限
  end;

  pTAffixGroup = ^TAffixGroup;
  TAffixGroup = packed record
    sGroupName: string[20];     // 词缀组名称
    AffixList: TList;           // 词缀列表
    nMinAffix: Integer;         // 最少词缀数
    nMaxAffix: Integer;         // 最多词缀数
  end;
  ...
  TDungeonTemplate = packed record
    nDungeonID: Word;           // 副本ID
    sDungeonName: string[30];   // 副本名称
    sMapName: string[16];       // 副本地图名
    nMinLevel: Integer;         // 最低等级
    nMaxLevel: Integer;         // 最高等级
    nMaxPlayers: Integer;       // 最大人数
    nMaxTime: LongWord;         // 最大时间(秒)
    nDailyLimit: Integer;       // 每日次数限制
    nEnterGold: Integer;        // 进入金币消耗
    nEnterGameGold: Integer;    // 进入元宝消耗
    DungeonType: TDungeonType;  // 副本类型
    sEnterMap: string[16];      // 进入地图
    nEnterX: Integer;           // 进入坐标X
    nEnterY: Integer;           // 进入坐标Y
    MonGenList: TList;          // 怪物生成列表
    BossList: TList;            // Boss列表
    RewardList: TList;          // 奖励列表
    sScript: string[50];        // 副本脚本
  end;

  pTDungeonInstance = ^TDungeonInstance;
  TDungeonInstance = packed record
    nInstanceID: LongWord;      // 实例ID
    nTemplateID: Word;          // 模板ID
    sMapName: string[16];       // 当前地图名
    nCurrX, nCurrY: Integer;    // 当前坐标
    dwCreateTime: LongWord;     // 创建时间
    dwEndTime: LongWord;        // 结束时间
    PlayerList: TList;          // 玩家列表
    MonsterList: TList;         // 怪物列表
    nState: Integer;            // 状态 0=等待,1=进行中,2=完成,3=失败
    nProgress: Integer;         // 进度 0-100
    nCurrentWave: Integer;      // 当前波次
    nMaxWave: Integer;          // 最大波次
    nKillCount: Integer;        // 杀怪计数
    nKillTarget: Integer;       // 杀怪目标
    nBossKillCount: Integer;    // Boss击杀数
    nDeathCount: Integer;       // 死亡次数
    nRewardMultiplier: Integer; // 奖励倍率
  end;
  ...
  TBehaviorNode = packed record
    nNodeID: Integer;           // 节点ID
    sNodeName: string[30];      // 节点名称
    NodeType: TBehaviorNodeType;// 节点类型
    sCondition: string[100];    // 条件表达式
    sAction: string[100];       // 动作表达式
    nWeight: Integer;           // 权重(随机节点使用)
    nMaxExecCount: Integer;     // 最大执行次数
    nCoolDown: LongWord;        // 冷却时间
    Children: TList;            // 子节点列表
  end;

  pTMonsterAIConfig = ^TMonsterAIConfig;
  TMonsterAIConfig = packed record
    sMonsterName: string[14];   // 怪物名称
    nMonsterID: Integer;        // 怪物ID
    RootNode: pTBehaviorNode;   // 行为树根节点
    nAggroRange: Integer;       // 警戒范围
    nChaseRange: Integer;       // 追击范围
    nReturnRange: Integer;      // 返回范围
    boSmartSkill: Boolean;      // 智能技能使用
    boCallHelp: Boolean;        // 呼唤同伴
    boFleeLowHP: Boolean;       // 低血量逃跑
    nFleeHPPercent: Integer;    // 逃跑血量百分比
    SkillList: TList;           // 技能列表
    sOnDeathScript: string[50]; // 死亡脚本
    sOnSpawnScript: string[50]; // 出生脚本
  end;
  ...
  TActivityInfo = packed record
    nActivityID: Word;          // 活动ID
    sActivityName: string[30];  // 活动名称
    sDescription: string[200];  // 活动描述
    ActivityType: TActivityType;// 活动类型
    sStartTime: string[10];     // 开始时间 HH:MM
    sEndTime: string[10];       // 结束时间 HH:MM
    nWeekDay: Byte;             // 星期几(0=每天,1-7=周一到周日)
    nMonthDay: Byte;            // 每月几号(0=不限)
    nMinLevel: Integer;         // 最低等级
    nMaxLevel: Integer;         // 最高等级
    nMinPlayers: Integer;       // 最少人数
    nMaxPlayers: Integer;       // 最大人数
    nDuration: LongWord;        // 持续时间(秒)
    nPrepareTime: LongWord;     // 准备时间(秒)
    sMapName: string[16];       // 活动地图
    nEnterX, nEnterY: Integer;  // 进入坐标
    sScript: string[50];        // 活动脚本
    boEnabled: Boolean;         // 是否启用
    boAutoStart: Boolean;       // 是否自动开始
    nRewardGold: Integer;       // 金币奖励
    nRewardGameGold: Integer;   // 元宝奖励
    nRewardExp: Integer;        // 经验奖励
    RewardItemList: TList;      // 物品奖励列表
  end;
  ...
  TMasterApprentice = packed record
    sMasterName: string[14];    // 师傅名称
    sApprenticeName: string[14];// 徒弟名称
    dwCreateTime: LongWord;     // 建立时间
    nMasterLevel: Integer;      // 建立时师傅等级
    nApprenticeLevel: Integer;  // 建立时徒弟等级
    nApprenticeCount: Integer;  // 徒弟已出师数量
    nMasterExpBonus: Integer;   // 师傅经验加成
    nApprenticeExpBonus: Integer; // 徒弟经验加成
    boCompleted: Boolean;       // 是否已出师
    dwCompleteTime: LongWord;   // 出师时间
    nCompleteLevel: Integer;    // 出师等级
    nDailyQuestCount: Integer;  // 每日任务完成数
    nMaxDailyQuest: Integer;    // 每日任务上限
    nMentorPoints: Integer;     // 师徒点数
    sLastQuestDate: string[10]; // 上次任务日期
  end;
  ...
  TGuildTech = packed record
    nTechID: Word;              // 科技ID
    sTechName: string[30];      // 科技名称
    TechType: TGuildTechType;   // 科技类型
    nMaxLevel: Integer;         // 最大等级
    nCurrentLevel: Integer;     // 当前等级
    nUpgradeCost: Integer;      // 升级消耗(金币)
    nUpgradeBuildPoint: Integer;// 升级消耗(建筑度)
    nUpgradeStability: Integer; // 升级消耗(安定度)
    nUpgradeTime: LongWord;     // 升级时间(秒)
    dwUpgradeStartTime: LongWord; // 升级开始时间
    boUpgrading: Boolean;       // 是否正在升级
    sDescription: string[100];  // 科技描述
    sEffect: string[100];       // 效果描述
    EffectValue: array[0..4] of Integer; // 每级效果值
  end;
  ...
  TSeasonInfo = packed record
    nSeasonID: Integer;         // 赛季ID
    sSeasonName: string[30];    // 赛季名称
    SeasonState: TSeasonState;  // 赛季状态
    dwStartTime: LongWord;      // 开始时间
    dwEndTime: LongWord;        // 结束时间
    nRankingType: Integer;      // 排名类型 0=等级,1=战力,2=PK,3=综合
    nMaxRank: Integer;          // 最大排名数
    RewardList: TList;          // 奖励列表
    sDescription: string[200];  // 赛季描述
  end;

  // === 新增 新手引导数据结构 ===
  pTGuideStep = ^TGuideStep;
  TGuideStep = packed record
    nStepID: Integer;           // 步骤ID
    sStepName: string[30];      // 步骤名称
    sDescription: string[200];  // 步骤描述
    nTargetType: Integer;       // 目标类型 0=NPC对话,1=杀怪,2=收集,3=到达,4=使用,5=装备
    sTargetName: string[20];    // 目标名称
    nTargetCount: Integer;      // 目标数量
    sMapName: string[16];       // 目标地图
    nTargetX, nTargetY: Integer;// 目标坐标
    nRewardExp: Integer;        // 经验奖励
    nRewardGold: Integer;       // 金币奖励
    nRewardItemID: Word;        // 物品奖励ID
    nRewardItemCount: Integer;  // 物品奖励数量
    sNextStep: string[20];      // 下一步骤
    boAuto: Boolean;            // 是否自动完成
    nEffectID: Integer;         // 特效ID
  end;

  pTNewbieGuide = ^TNewbieGuide;
  TNewbieGuide = packed record
    sGuideName: string[30];     // 引导名称
    StepList: TList;            // 步骤列表
    nCurrentStep: Integer;      // 当前步骤
    boCompleted: Boolean;       // 是否完成
    nTotalRewardExp: Integer;   // 总经验奖励
    nTotalRewardGold: Integer;  // 总金币奖励
  end;
