...
//    m_boDenyRefStatus: Boolean; //是否刷新在地图上信息；
    ...
    // === 新增: GCD冷却系统字段 ===
    m_GCDList: TList;                   // GCD冷却列表(TGCDInfo)
    m_dwGlobalGCDTick: LongWord;        // 全局GCD计时
    m_nGlobalGCDTime: LongWord;         // 全局GCD时间(ms)

    // === 新增: BUFF系统字段 ===
    m_BuffList: TList;                  // BUFF列表(由BuffManager管理)

    // === 新增: 词缀系统字段 ===
    m_ItemAffixList: TList;             // 装备词缀缓存列表

    // === 新增: 自动战斗字段 ===
    m_boAutoFight: Boolean;             // 是否自动战斗
    m_dwAutoFightTick: LongWord;         // 自动战斗计时
    m_nAutoFightRange: Integer;         // 自动战斗范围
    m_boAutoUseSkill: Boolean;          // 自动使用技能
    m_boAutoUseItem: Boolean;           // 自动使用药品
    m_nAutoHPPercent: Integer;          // 自动吃药HP百分比
    m_nAutoMPPercent: Integer;          // 自动吃药MP百分比

    // === 新增: 新手引导字段 ===
    m_Guide: TNewbieGuide;              // 新手引导数据
    m_boGuideCompleted: Boolean;        // 引导是否完成
    m_nGuideProgress: Integer;          // 引导进度

    // === 新增: 赛季系统字段 ===
    m_nSeasonScore: Integer;            // 赛季积分
    m_nSeasonRank: Integer;             // 赛季排名
    m_nSeasonLevel: Integer;            // 赛季等级
    m_nSeasonKillCount: Integer;        // 赛季击杀数
    m_nSeasonWinCount: Integer;         // 赛季胜场数
    m_nSeasonParticipation: Integer;    // 赛季参与次数
//    m_btWuXinLevel: Byte;
