...
// MARK: - Step 12: 多机位拍摄角度
struct CameraAngle {
    let title: String
    let voiceHint: String
    let requiredPitch: Double?   // 要求的俯仰设备值，比如 0.2 (仰拍)，-0.1 (俯拍)，0.0 (平视)等。为空表示无强制约束。
    // 在这个机位下可能的基准骨骼点替代集（人换位置后骨骼二维点会发生形变），如果 nil 就延用 Plan 的初始值
    let posePoints: [String: CGPoint]?
}

// MARK: - Step 13: Vlog 录播引擎模型
struct VlogClip {
    let duration: TimeInterval     // 这个分镜拍几秒
    let voiceCommand: String       // 拍摄前或期间的语音倒数指导
    let overlayText: String        // 大字板展现的情绪字幕
}

struct VlogTemplate {
    let bgmFilename: String?       // 最终合成的背板音乐，如 "vlog_beat.mp3"
    let clips: [VlogClip]          // 分发场记表
}
...
struct ShootingPlan: Identifiable {
    ...
    let secondaryPosePoints: [String: CGPoint]? // 第二个人的对应骨骼关键点（仅双人模式非空）
    let sequence: [ActionFrame]?                // Step 10: 连续多帧连拍方案
    let multiAngles: [CameraAngle]?             // Step 12: 面向摄影师移动的多机位排演
    let vlogScript: VlogTemplate?               // Step 13: 以 Vlog 录像为主的极客分镜表。互斥优先级：Vlog > Sequence > MultiAngle
    
    init(id: String, poseName: String, poseEmoji: String, poseDescription: String, composition: CompositionRule, frameRatio: FrameRatio, voiceGuide: String, posePoints: [String: CGPoint], secondaryPosePoints: [String: CGPoint]? = nil, sequence: [ActionFrame]? = nil, multiAngles: [CameraAngle]? = nil, vlogScript: VlogTemplate? = nil) {
        self.id = id
        ...
        self.secondaryPosePoints = secondaryPosePoints
        self.sequence = sequence
        self.multiAngles = multiAngles
        self.vlogScript = vlogScript
    }
}
...
final class PoseLibrary {
    ...
    private var coffeePlans: [ShootingPlan] = [
        ShootingPlan(
            ...
            posePoints: [
                ...
                "leftHip":       CGPoint(x: 0.46, y: 0.60),
                "rightHip":      CGPoint(x: 0.54, y: 0.60)
            ],
            vlogScript: VlogTemplate(bgmFilename: nil, clips: [
                VlogClip(duration: 3.0, voiceCommand: "好，向镜头走两步！", overlayText: "▶ 向前散步..."),
                VlogClip(duration: 4.0, voiceCommand: "停！顺着肩膀转个圈", overlayText: "▶ 轻轻转身回眸"),
                VlogClip(duration: 2.0, voiceCommand: "对，保持这个笑容，完美！", overlayText: "▶ 录制定格")
            ])
        ),
        ShootingPlan(
            ...
            multiAngles: [
                CameraAngle(title: "默认平拍", voiceHint: "第一拍，轻松捧杯，保持不动。", requiredPitch: nil, posePoints: nil),
                CameraAngle(title: "下蹲仰拍", voiceHint: "摄影师请蹲下，尝试极其出片的高级仰拍！", requiredPitch: 0.2, posePoints: nil) // 俯仰角要求大于0.2（仰角）
            ]
        ),
        ShootingPlan(
            id: "coffee_lean",
    ...
    private var parkPlans: [ShootingPlan] = [
        // 连拍方案：少女三连拍
        ShootingPlan(id: "park_seq_3", poseName: "[连拍]少女三连", poseEmoji: "🏃‍♀️", poseDescription: "包含三个连续动作，自动连拍合成动图", composition: .center, frameRatio: .halfBody, voiceGuide: "开启三连拍模式，站在中间，听我口令完成三个动作。",
            posePoints: [
                "neck": CGPoint(x: 0.50, y: 0.30), "leftShoulder": CGPoint(x: 0.40, y: 0.40), "rightShoulder": CGPoint(x: 0.60, y: 0.40)
            ],
            sequence: [
                ActionFrame(emoji: "✌️", title: "第一拍: 剪刀手", voiceHint: "第一拍，举起右手比剪刀手", posePoints: [
                    "neck": CGPoint(x: 0.50, y: 0.30), "leftShoulder": CGPoint(x: 0.40, y: 0.40), "rightShoulder": CGPoint(x: 0.60, y: 0.40),
                    "rightElbow": CGPoint(x: 0.70, y: 0.30), "rightWrist": CGPoint(x: 0.60, y: 0.15)
                ]),
                ActionFrame(emoji: "🙆‍♀️", title: "第二拍: 抱头", voiceHint: "很好，切第二拍，双手抱头", posePoints: [
                    "neck": CGPoint(x: 0.50, y: 0.30), "leftShoulder": CGPoint(x: 0.40, y: 0.40), "rightShoulder": CGPoint(x: 0.60, y: 0.40),
                    "leftElbow": CGPoint(x: 0.30, y: 0.20), "rightElbow": CGPoint(x: 0.70, y: 0.20),
                    "leftWrist": CGPoint(x: 0.45, y: 0.10), "rightWrist": CGPoint(x: 0.55, y: 0.10)
                ]),
                ActionFrame(emoji: "🫶", title: "第三拍: 比心", voiceHint: "最后一张，双手在胸口比心", posePoints: [
                    "neck": CGPoint(x: 0.50, y: 0.30), "leftShoulder": CGPoint(x: 0.40, y: 0.40), "rightShoulder": CGPoint(x: 0.60, y: 0.40),
                    "leftElbow": CGPoint(x: 0.45, y: 0.50), "rightElbow": CGPoint(x: 0.55, y: 0.50),
                    "leftWrist": CGPoint(x: 0.48, y: 0.40), "rightWrist": CGPoint(x: 0.52, y: 0.40)
                ])
            ]
        ),
        // 双人方案：背靠背
        ShootingPlan(id: "park_dual_back", poseName: "[双人]草地背靠背", poseEmoji: "👯", poseDescription: "两人背靠背坐在草地上，画面温馨。适合情侣或闺蜜。", composition: .center, frameRatio: .fullBody, voiceGuide: "两人走到画面中间，背靠着背坐下，手随意放在膝盖上。",
            posePoints: [
                // 左侧人物 (面向左方)
                "neck": CGPoint(x: 0.40, y: 0.45), "leftShoulder": CGPoint(x: 0.32, y: 0.52), "rightShoulder": CGPoint(x: 0.43, y: 0.52),
                "leftHip": CGPoint(x: 0.38, y: 0.70), "rightHip": CGPoint(x: 0.45, y: 0.70), "leftKnee": CGPoint(x: 0.25, y: 0.85), "rightKnee": CGPoint(x: 0.40, y: 0.85)
            ],
            secondaryPosePoints: [
                // 右侧人物 (面向右方)
                "neck": CGPoint(x: 0.60, y: 0.45), "leftShoulder": CGPoint(x: 0.57, y: 0.52), "rightShoulder": CGPoint(x: 0.68, y: 0.52),
                "leftHip": CGPoint(x: 0.55, y: 0.70), "rightHip": CGPoint(x: 0.62, y: 0.70), "leftKnee": CGPoint(x: 0.60, y: 0.85), "rightKnee": CGPoint(x: 0.75, y: 0.85)
            ]
        ),
    ...
    func classify(pixelBuffer: CVPixelBuffer, completion: @escaping (SceneType) -> Void) {
        ...
        let request = VNCoreMLRequest(model: mlModel) { req, _ in
            ...
            for obs in topResults {
                ...
                if cityKeywords.contains(where:   { id.contains($0) })  { votes[.city_street]!  += w }
                if parkKeywords.contains(where:   { id.contains($0) })  { votes[.park]!         += w }
                if indoorKeywords.contains(where: { id.contains($0) })  { votes[.indoor_home]!  += w }
                if neonKeywords.contains(where:   { id.contains($0) })  { votes[.neon_night]!   += w }
            }
